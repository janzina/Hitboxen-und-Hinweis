...
public class Background {
    ...
    private int[][]     layer1Map;      // ── nur Layer 1
    private int mapRows;
    private int mapCols;
    ...
    public void loadMapWithLayers() {
        ...
        buildLayer1Map(layerLines.get(1));  // ── Layer 1 als ID-Map speichern
    ...
    private void buildLayer1Map(List<String> layer1Lines) {
        layer1Map = new int[mapRows][mapCols];
        for (int row = 0; row < layer1Lines.size() && row < mapRows; row++) {
            String[] tokens = layer1Lines.get(row).trim().split("\\s+");
            for (int col = 0; col < tokens.length && col < mapCols; col++) {
                layer1Map[row][col] = Integer.parseInt(tokens[col].trim());
            }
        }
    }

    public int getLayer1TileIdAt(double worldX, double worldY) {
        int col = (int)(worldX / tileSize);
        int row = (int)(worldY / tileSize);
        if (row < 0 || row >= mapRows || col < 0 || col >= mapCols) return -1;
        return layer1Map[row][col];
    }

    public boolean isWaterAt(double worldX, double worldY) {
        return getLayer1TileIdAt(worldX, worldY) == 1;
    }
    ...
    public boolean isWalkableTile(int row, int col) {
        if (row < 0 || row >= mapRows) return false;
        if (col < 0 || col >= mapCols) return false;
        return collisionMap[row][col];
    }
}